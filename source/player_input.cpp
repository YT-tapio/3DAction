...
const bool PlayerInput::IsPush(ConfigName name) const
{
	...
	// ダッシュに対応されているボタンを見る
	// padはbボタン長押しで
	auto config_data = ConfigDatas::GetInstance().GetConfigData(name);
	for (auto& input : inputs_)
	{
		auto pad = std::dynamic_pointer_cast<Pad>(input);
...
const bool PlayerInput::IsDash() const
{
	...
	// ダッシュに対応されているボタンを見る
	// padはbボタン長押しで
	auto config_data = ConfigDatas::GetInstance().GetConfigData(ConfigName::dash);
	for (auto& input : inputs_)
	{
		auto pad = std::dynamic_pointer_cast<Pad>(input);
		float pushing_time = -1.f;
...
const bool PlayerInput::IsJump() const
{
	if (is_stop_) { return FALSE; }
	// 攻撃に対応されているボタンを見る
	// padはbボタン長押しで
	auto config_data = ConfigDatas::GetInstance().GetConfigData(ConfigName::jump);
	for (auto& input : inputs_)
	{
		auto pad = std::dynamic_pointer_cast<Pad>(input);
		float pushing_time = -1.f;
...
const bool PlayerInput::IsLockOnEnemy() const
{
	...
	// 攻撃に対応されているボタンを見る
	// padはbボタン長押しで
	auto config_data = ConfigDatas::GetInstance().GetConfigData(ConfigName::lock_on);
	for (auto& input : inputs_)
	{
		auto pad = std::dynamic_pointer_cast<Pad>(input);
		float pushing_time = 0.f;
...
const bool PlayerInput::GoNextScene() const
{
	...
	for (auto& input : inputs_)
	{
		...
		if (pad != nullptr)
		{
			pushing_time = pad->GetPushingTimeButton(PadConfig::go_next);
			if (pushing_time == 0.f) { return TRUE; }
		}
		else
		{
			...
			if (pc != nullptr)
			{
				pushing_time = pc->GetPushingTimeKey(KeyConfig::go_next);
...
const bool PlayerInput::GoResult() const
{
	...
	for (auto& input : inputs_)
	{
		...
		if (pad != nullptr)
		{
			pushing_time = pad->GetPushingTimeButton(PadConfig::go_result);
			if (pushing_time == 0.f) { return TRUE; }
		}
		else
		{
			...
			if (pc != nullptr)
			{
				pushing_time = pc->GetPushingTimeKey(KeyConfig::go_result);
